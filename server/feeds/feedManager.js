...
class FeedManager {
  ...
  async getLatestTick(symbol) { const sym=String(symbol||'').toUpperCase();
    for (const source of this.activeProviders){ const provider=providerRegistry.get(source); if(provider?.getLatestTick){ const live = await provider.getLatestTick(sym); if(live) return this.ingestTick(live); } }
    return this.latestTicks.get(sym) || null; }
  async getLatestCandle(symbol,timeframe='1m'){ const sym=String(symbol||'').toUpperCase();
    for (const source of this.activeProviders){ const provider=providerRegistry.get(source); if(provider?.getLatestCandle){ const live = await provider.getLatestCandle(sym,timeframe); if(live) return this.ingestCandle(live); } }
    const key=`${sym}:${timeframe}`; const cached=this.latestCandles.get(key); if(cached) return cached;
    const fromStore=getCandlesWithMeta(symbol,timeframe); const candles=fromStore?.candles||[]; if(!candles.length) return null; const last=candles[candles.length-1]; return createNormalizedCandle({symbol,timeframe,open:last.o,high:last.h,low:last.l,close:last.c,volume:last.v,source:fromStore.source||'fallback_demo',timestamp:last.t}); }
