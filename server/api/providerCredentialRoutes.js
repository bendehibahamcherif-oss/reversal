...
function validationError(message, details = []) {
  return { ok: false, success: false, error: { code: 'validation_error', message, details } };
}
...
function saveCredential(req, res) {
  try {
    ...
    if (!provider) {
      return res.status(400).json({ ok: false, success: false, error: { code: 'unknown_provider', message: `Unknown provider '${providerId}'.`, providerId } });
    }
    if (!apiKey) {
    ...
    return res.status(200).json({ ok: true, success: true, provider: canonical, credentials: credential });
  } catch {
    return res.status(500).json({ ok: false, success: false, error: { code: 'credential_save_failed', message: 'Unable to save provider credentials.' } });
  }
}

providerCredentialRoutes.get('/credentials', (_req, res) => {
  try {
    return res.json({ ok: true, success: true, credentials: credentialsMap() });
  } catch {
    return res.status(500).json({ ok: false, success: false, error: { code: 'credential_list_failed', message: 'Unable to load provider credentials.' } });
  }
});
...
providerCredentialRoutes.delete('/credentials/:providerId', (req, res) => {
  try {
    ...
    return res.json({ ok: true, success: true, provider: canonicalProvider(provider.id), credentials: providerCredentialStatus(provider.id), ...feedManager.getCanonicalProviderState() });
  } catch {
    return res.status(500).json({ ok: false, success: false, error: { code: 'credential_delete_failed', message: 'Unable to delete provider credentials.' } });
  }
});

providerCredentialRoutes.get('/status', (_req, res) => {
  res.json({ ok: true, success: true, providers: feedManager.getCanonicalProviderState().providers });
});

providerCredentialRoutes.get('/active', (_req, res) => {
  const result = feedManager.getActiveProviders();
  res.json({ ok: true, success: true, ...result, activeProviders: result.providers });
});

providerCredentialRoutes.post('/active', (req, res) => {
  const result = feedManager.saveActiveProviders(req.body || {});
  if (!result.ok) {
    return res.status(result.status || 400).json({ ok: false, success: false, error: result.error });
  }
...
providerCredentialRoutes.get('/runtime', (_req, res) => {
  res.json({ ok: true, success: true, providers: feedManager.listProviders().map((provider) => ({ id: provider.id, runtime: provider.runtime })) });
});
...
providerCredentialRoutes.get('/health', (_req, res) => {
  res.json({ ok: true, ...feedManager.getCanonicalProviderState() });
});
